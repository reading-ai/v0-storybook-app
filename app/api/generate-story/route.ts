...
Format the response as a complete chapter with proper paragraphs and markdown formatting.`
              ...
              controller.enqueue(
                encoder.encode(
                  `event: text\ndata: ${JSON.stringify({
                    ...
                    fullContent,
                    wordCount,
                    progress: Math.min(90, (wordCount / 100) * 90), // Estimate progress based on word count
                  })}\n\n`,
                ),
              )
            ...
            controller.enqueue(
              encoder.encode(
                `event: error\ndata: ${JSON.stringify({
                  ...
                  fallback: generateTemplateChapter(chapterNumber, characters, setting, genre, prompt),
                })}\n\n`,
              ),
            )
